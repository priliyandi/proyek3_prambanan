...
class sistem extends CI_Controller {
	...
	public function register() {
		...
		$this->form_validation->set_rules('email_user', 'Email', 'required|trim|valid_email|is_unique[tbl_user.email_user]', [
			'is_unique' => 'Email sudah terdaftar, buat email baru!'
		]);
		...
		if ($this->form_validation->run()==FALSE) {
			...
			$data['tentang_kami'] = $this->sistem_model->TentangKami();
			$this->load->view('sistem/register',$data);
			...
			$data = array('nama_user'		=> $i->post('nama_user'),
						  ...
						  'telp_user'		=> $i->post('telp_user'),
						  'username_user'	=> $i->post('username_user'),
						  'password_user'	=> md5($i->post('password_user')),
						  'user_group_id' 	=> 3
						);
			$this->sistem_model->CekRegis($data);
	...
	//Awal Kamar
	public function kamar_home() {

		$data['tentang_kami'] 	= $this->sistem_model->TentangKami();
		$data['kamar'] 			= $this->sistem_model->KamarAll();
		$data['kelas_kamar'] 	= $this->sistem_model->KelasKamar();

		$this->template_home->load('template_system','sistem/data/kamar',$data);
	...
	public function kamar_kelas_kamar () {
		...
		$data['tentang_kami'] 	= $this->sistem_model->TentangKami();
		$data['kamar'] 			= $this->sistem_model->KamarKelasKamar($id);
		$data['kelas_kamar'] 	= $this->sistem_model->KelasKamar();

		$this->template_home->load('template_system','sistem/data/kamar_kelas_kamar',$data);

	}

	public function kamar_detail_home () {

		$id = $this->uri->segment(3);

		$data['tentang_kami'] 	= $this->sistem_model->TentangKami();
		$data['kamar'] 			= $this->sistem_model->KamarDetail($id);
		$data['kamar_gambar'] 	= $this->sistem_model->KamarGambarId($id);
		$data['kelas_kamar'] 	= $this->sistem_model->KelasKamar();

		$this->template_home->load('template_system','sistem/data/kamar_detail',$data);

	}

	public function reservasi_home() {
			$this->form_validation->set_rules('tgl_reservasi_masuk','Tanggal Masuk','required');
			$this->form_validation->set_rules('tgl_reservasi_keluar','Tanggal Keluar','required');
			$this->form_validation->set_rules('nama_reservasi','Nama','required');
			...
			if ($this->form_validation->run()==FALSE) {
				...
				$this->template_sistem->load('template_system','sistem/data/kamar_detail',$data);
					...
					$this->session->set_flashdata('berhasil','OK');
					$id = $this->input->post('id_kamar');
					redirect("sistem/kamar_detail/".$id);
			}
	}
	//Akhir Kamar
