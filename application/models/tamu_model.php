...
class tamu_model extends CI_Model {
	...
	function CekLogin($username_user,$password_user) {
		...
		$ceklogin = $this->db->query("select a.*,b.* from tamu a join tbl_user_group b on a.user_group_id=b.id_user_group where a.email_tamu='$email_tamu' and a.password_tamu='$password_tamu' ");
		...
		if (count($ceklogin->result())>0) {
			...
			foreach ($ceklogin->result() as $value) {
				...
				$sess_data['id_tamu']  			= $value->id_user;
				$sess_data['nama_tamu']  		= $value->nama_user;
				$sess_data['email_tamu']  		= $value->email_user;
				$sess_data['password_tamu']  	= $value->password_user;
				$sess_data['telepon_tamu']  	= $value->telp_user;
				$sess_data['telepon_tamu']  	= $value->telp_user;
